...
- Use photography only for a single personal portrait in the portfolio hero. Keep all other sections image-light and led by type, icons, and line work.
...
Near-zero photography. For the personal portfolio adaptation, one portrait is permitted in the hero and should sit inside a restrained Graphite hairline frame with no shadow or decorative treatment. Dot-matrix graphics may still be used as secondary atmosphere, but no product shots, team photos, or decorative photography should appear elsewhere. Icons remain the recurring graphic motif: thin 1.5px outlined strokes in Compass Gold or Chalk, geometric and minimal. The visual system still treats whitespace and type as the primary content.

## Layout

Full-bleed Obsidian canvas, content constrained to a 1200px max-width centered column. In the personal portfolio adaptation, the hero uses a two-column editorial composition: primary introduction and calls-to-action on the left, a single framed portrait on the right, stacking cleanly on mobile. Sections are separated exclusively by 1px Graphite horizontal dividers — no alternating bands, no background shifts. Service/skill groups use structured grids inside hairline frames. Navigation remains visually transparent and restrained. Spacing is generous — 120–210px between sections — creating a slow, editorial vertical rhythm rather than a dense product layout.
